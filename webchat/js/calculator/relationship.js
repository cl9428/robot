...
(function(){
	var $type = {checked:true};
	var $sex = {checked:true};
	var $reverse = {checked:true};
	...
	var count = function(){
		...
		if(value){
			var sex = $sex.checked?1:0;
			var type = $type.checked?'default':'chain';
	...
	for(var i=0;i<$btns.length;i++){
		$btns[i].onclick = function(){
			...
			if(name == "老公" || name == "老婆"){
				var mark = $(this).hasClass("on1");
				if(mark){
					return
				}else{
					if(name == "老婆"){
						$(".btn-warpper .v1-btn").eq(6).removeClass("on1");
						$(".btn-warpper .v1-btn").eq(7).addClass("on1");
					}else{
						$(".btn-warpper .v1-btn").eq(6).addClass("on1");
						$(".btn-warpper .v1-btn").eq(7).removeClass("on1");
					}
				}
			}
			var isMan = $(this).hasClass("man"),isWoman = $(this).hasClass("woman");
			if(isMan){
				$(".btn-warpper .v1-btn").eq(6).addClass("on1");
				$(".btn-warpper .v1-btn").eq(7).removeClass("on1");
			}else if(isWoman){
	...
	$(".b1-btn-header .l_lebel").click(function(){
		var mark = $(this).find("i").hasClass("on");
		var i = $(this).index(),txt = $(this).text();
		console.log(txt);
		if(!mark){
			$(this).find("i").addClass("on").parent().siblings().find("i").removeClass("on");
				$(".btn-warpper .spouse").eq(txt == "男"?0:1).addClass("on1").siblings().removeClass("on1");
		}
		$sex.checked = i == 0?true:false;
	});
	$(".reverse").click(function(){
		var mark = $(this).hasClass("on");
		$(this)[mark?"removeClass":"addClass"]("on");
		$("#input").attr("placeholder",mark ? "我称呼Ta" : "Ta称呼我");
		$reverse.checked = mark?true:false;
		if($("#reslut").text()){
			count();
		}
	});
	
	$("#v1-seach").click(function(){
		var value = $("#v1-inp").val().trim();
		if(!value){
			plus.nativeUI.toast("请输入对方称谓");
			return;
		}
		var result = relationship({text:value,type:"chain"});
		$("#reslut1").text(result[0]);
	});
